...
{
    /// <summary>
    /// AES-OFB（输出反馈模式）对称加密实现，依赖 BouncyCastle。
    /// OFB 适合流式加密，块大小为 16 字节，密钥为 SHA256(key)。
    /// </summary>
    public class AesOfbDataProvider : ISymmetricAlgorithm
    {
        /// <summary>
        /// 使用 AES-OFB 模式加密明文。
        /// </summary>
        ...
        public string Encrypt(string plainText, string key)
        {
            ...
            // OFB 模式，分组大小为 128 位（16 字节）
            var cipher = new BufferedBlockCipher(new OfbBlockCipher(new AesEngine(), blockSize * 8));

...
{
    /// <summary>
    /// Camellia 对称加密算法实现（CBC模式，PKCS7填充，IV+密文Base64输出）。
    /// 依赖 BouncyCastle。Camellia 支持 128/192/256 位密钥，块大小为 16 字节。
    /// </summary>
    public class CamelliaDataProvider : ISymmetricAlgorithm
    {
        /// <summary>
        /// 使用 Camellia 算法加密明文。
        /// </summary>
        /// <param name="plainText">要加密的明文字符串。</param>
        /// <param name="key">密钥（建议长度 16/24/32 字节，超长会被截断）。</param>
        /// <returns>Base64(IV+密文)。</returns>
        public string Encrypt(string plainText, string key)
        {
            if (string.IsNullOrEmpty(plainText)) throw new ArgumentNullException(nameof(plainText));
            if (string.IsNullOrWhiteSpace(key)) throw new ArgumentNullException(nameof(key));
            // Camellia 密钥长度 16/24/32 字节，超长自动截断
            var keyBytes = Encoding.UTF8.GetBytes(key);
            if (keyBytes.Length > 32) keyBytes = keyBytes[..32];
            var engine = new CamelliaEngine();
            var blockSize = engine.GetBlockSize(); // 16 字节
            // 生成随机 IV

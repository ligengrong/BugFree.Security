...
{
    /// <summary>使用 DES (CBC 模式) 提供数据保护</summary>
    public class DesDataProvider : ISymmetricAlgorithm
    {
        //使用ThreadLocal缓存Aes实例以提高性能，确保线程安全。
        static readonly ThreadLocal<DES> _Des = new(() =>
        {
            var des = DES.Create();
        ...
        /// <summary>使用 DES (CBC 模式) 加密明文。</summary>
        ...
        /// <summary>使用 DES (CBC 模式) 解密密文。</summary>
